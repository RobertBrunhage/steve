# Steve Browser Architecture

This document defines the browser model Steve is actually building now.
...
- Keep one Steve-owned browser tool surface for agents.

## Product Shape

Steve now has two browser targets:

- `container`
  - default
  - Playwright + Chromium inside the main `steve` container
...
- `remote`
  ...
  - Steve talks to a small host-side companion
...
- no Steve-managed host Chrome profiles
- no screenshot-polling host viewer
- no custom click/keyboard forwarding page

Instead, Steve keeps the strong container path and adds a simpler attached-browser path.

## Remote Attach Model

The `remote` target means:

1. Steve runs in Docker as usual.
2. A lightweight host-side remote browser companion runs outside Docker.
3. When Steve needs the `remote` target, it calls that companion over HTTP.
4. The companion attaches to the operator's local Chrome session through Chrome DevTools MCP.
5. Steve continues using the same top-level browser actions and snapshot loop.

This gives Steve a real local browser without making the agent deal with raw CDP or custom Playwright code.

## Per-User Attachment

Attached browser access is per user, not global.

Each Steve user can optionally have:
...
Dashboard settings do not need browser mode controls. Steve always uses the container browser by default.
...
6. When Steve needs the attached browser, Chrome shows the approval prompt.

## Auth Flow

The intended real-world flow is:

1. User asks Steve to go to `x.com`.
2. Steve starts in `container` by default.
3. If the site is auth-heavy or the container browser is rejected, Steve escalates to `remote` when that user has an attached browser configured.
4. Steve opens the page in the attached local Chrome session.
5. The user signs in or approves prompts in that real Chrome window.
6. The user says they are done.
7. Steve continues using the attached browser state.

If a site only needs normal browsing, Steve stays in `container` and can still return the container viewer URL.
...
For `remote`, Steve maps Chrome DevTools MCP snapshot uids into the same Steve browser ref flow.

## Backend Selection

Steve, not the model, chooses the target.
...
If the remote companion or attached browser is unavailable, Steve should return a clear error when `remote` is explicitly required.

## Security Model

- `container` remains Steve's isolated baseline browser.
...
- remote companion access stays local/private to the Steve install
...
Steve should reliably do all of these:
